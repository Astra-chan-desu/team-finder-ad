...
class ProjectViewsTest(TestCase):
    ...
    def test_favorite_add(self):
        ...
        response = self.client.post(url)
        self.assertEqual(response.status_code, 200)
        self.assertTrue(Favorite.objects.filter(user=self.other_user, project=self.project).exists())
    ...
    def test_favorite_remove(self):
        ...
        response = self.client.post(url)
        self.assertEqual(response.status_code, 200)
        self.assertFalse(Favorite.objects.filter(user=self.other_user, project=self.project).exists())
...
class AdminPermissionsTest(TestCase):
    def setUp(self):
        ...
        self.project = Project.objects.create(name='Проект', description='...', owner=self.user)
